...
def server(remote_ip, remote_port, local_cid, local_port):
    try:
        dock_socket = socket.socket(socket.AF_VSOCK, socket.SOCK_STREAM)
        dock_socket.bind((local_cid, local_port))
        ...
        new_thread = threading.Thread(target=server,
                                      args=(remote_ip, remote_port, local_cid,
                                            local_port))
        new_thread.start()
...
def main(args):
    remote_ip = str(args[0])
    remote_port = int(args[1])
    local_cid = int(args[2])
    ...
    thread.start()
    print(
        f"starting forwarder on {remote_ip}:{remote_port} {local_cid}:{local_port}"
    )
    while True:
        time.sleep(60)
